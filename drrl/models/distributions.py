...
"copy from stable_baselines3/common/distributions.py"
...
class DiagGaussianDistribution(Distribution):
    ...
    def proba_distribution(self: SelfDiagGaussianDistribution, mean_actions: th.Tensor,
                           log_std: th.Tensor) -> SelfDiagGaussianDistribution:
        """
...
class SquashedDiagGaussianDistribution(DiagGaussianDistribution):
    ...
    def proba_distribution(self: SelfSquashedDiagGaussianDistribution, mean_actions: th.Tensor,
...
class MultiCategoricalDistribution(Distribution):
    ...
    def proba_distribution(self: SelfMultiCategoricalDistribution,
    ...
    def log_prob(self, actions: th.Tensor) -> th.Tensor:
        ...
        return th.stack([dist.log_prob(action) for dist, action in zip(self.distribution, th.unbind(actions, dim=1))],
                        dim=1).sum(dim=1)
...
class StateDependentNoiseDistribution(Distribution):
    ...
    def proba_distribution(self: SelfStateDependentNoiseDistribution, mean_actions: th.Tensor, log_std: th.Tensor,
...
def make_proba_distribution(action_space: spaces.Space,
                            use_sde: bool = False,
    ...
    if isinstance(action_space, spaces.Box):
        ...
        assert isinstance(
            action_space.n,
            int), f"Multi-dimensional MultiBinary({action_space.n}) action space is not supported. You can flatten it instead."
