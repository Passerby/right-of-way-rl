...
from drrl.envs.meta_drive import make_ai_vs_bot_env
...
def seperate_training(cfg, rank):
    train_server = TrainingServer(cfg, rank)
    train_server.run()

def run_multi_gpu_seperate_training(cfg) -> None:
    # rank 0 -> right, first_0 player
    # rank 1 -> left, second_0 player or bot
    world_size = torch.cuda.device_count() - 2
    for rank in range(world_size):
        p = Process(target=seperate_training, args=(cfg, rank))
        p.start()
...
@hydra.main(config_path="../drrl/configs", config_name="sac", version_base="1.3.0")
def main(cfg):
    ...
    for _ in range(16):
        p4 = Process(target=runner, args=(cfg,))
        p4.start()
